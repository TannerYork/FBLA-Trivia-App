...
class FourVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        FirestoreData.shared.getNationalOfficerQuestions { (bool) in
            if bool == false {
    ...
    func getQuestion() {
        if NationalOfficers.shared.questions.count > 0 {
            let index = Int.random(in: 0..<NationalOfficers.shared.questions.count)
            setColors()
            currentQuestion = NationalOfficers.shared.questions[index]
            NationalOfficers.shared.questions.remove(at: index)
