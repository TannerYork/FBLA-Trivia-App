...
class FiveVC: UIViewController {
        ...
        override func viewWillAppear(_ animated: Bool) {
            ...
            FirestoreData.shared.getParliamentaryQuestions { (bool) in
                if bool == false {
        ...
        func getQuestion() {
            if ParliamenaryProcedure.shared.questions.count > 0 {
                let index = Int.random(in: 0..<ParliamenaryProcedure.shared.questions.count)
                setColors()
                currentQuestion = ParliamenaryProcedure.shared.questions[index]
                ParliamenaryProcedure.shared.questions.remove(at: index)
